...
- Payments: web = Stripe, iOS = Apple IAP via RevenueCat (`react-native-purchases`, entitlement `pro`, products `maqraa_pro_monthly`/`maqraa_pro_yearly`). The SDK keys `entitlements.active` by the entitlement's **lookup key**, not its display name - `pro` is the lookup key, "Pro" the display name, and a mismatch there lets a purchase succeed while granting nothing. Likewise the paywall only has something to sell when the `default` offering's `$rc_monthly` / `$rc_annual` packages have the App Store products attached; an offering with empty packages is what got build 19 rejected under Guideline 2.1(a). Check both with the RevenueCat CLI (`npm i -g @revenuecat/cli`), which needs an explicit `--project-id projaaf0d77b` under `--no-input`: `rc offerings verify ofrngd477f2fe3a` for the whole graph, or curl the SDK's own endpoint (`/v1/subscribers/<any-id>/offerings` with the public `appl_` key and `X-Platform: ios`) to see exactly what a device would receive. RevenueCat wants two different Apple keys, generated on different tabs of App Store Connect > Users and Access > Integrations: an **App Store Connect API** key to import products, and an **In-App Purchase** key to record StoreKit 2 transactions. Both carry the same account-level issuer ID, so that is no way to tell them apart - check which tab a key was generated on. Both webhooks upsert the single `subscriptions` row; each sync refuses to overwrite the other provider's still-live sub. `getPlan` is provider-agnostic — don't fork entitlement logic.
