...
- Releasing runs from GitHub Actions, not a laptop: `ios-release` (build, optional submit) and `ios-ota` (JS-only update). Both are `workflow_dispatch`. They need an `EXPO_TOKEN` repo secret and an App Store Connect API key on EAS — that key lives under **account** settings > Credentials, not the project credentials page, which only offers manual `.p12` upload. With the key in place, a build on the `production-ci` profile (`credentialsSource: "remote"`) makes its own distribution certificate and provisioning profile; nothing needs uploading by hand. The laptop-only `production` profile still reads local credentials and is untouched. `EXPO_PUBLIC_*` must be set as EAS environment variables — EAS builds inline them, GitHub secrets never reach that step.
