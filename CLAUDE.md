...
- Releasing runs from GitHub Actions, not a laptop: `ios-release` (build, optional submit) and `ios-ota` (JS-only update). Both are `workflow_dispatch`. They need an `EXPO_TOKEN` repo secret, signing credentials stored on EAS (the `production-ci` build profile uses `credentialsSource: "remote"`; the laptop-only `production` profile still reads local ones), and an ASC API key registered with EAS. `EXPO_PUBLIC_*` must be set as EAS environment variables — EAS builds inline them, GitHub secrets never reach that step.
