  ...
  input_schema: {
    ...
    properties: {
      ...
      chapters: {
        type: "array",
        description:
          "The book's REAL reading chapters. The input is a list of numbered pages that may be arbitrary splits (often each page is a fragment, not a chapter). Group consecutive pages into the actual chapters by spotting chapter openings/headings (e.g. 'Chapter', 'الفصل', 'الباب', 'الحديث', a roman numeral, a title line). EXCLUDE front/back matter — cover, title page, publisher/copyright/ISBN/contact pages, dedication, table of contents, and 'about the author' — by simply NOT including those pages in any chapter. The chapters you return MUST form ONE contiguous block of pages with no gaps between them: each entry covers first_page..last_page inclusive, and each next chapter starts exactly at the previous last_page+1. The block may start after page 1 and end before the last page (that's how junk is dropped), but never leave a gap in the MIDDLE. If the pages already are real chapters, return one entry per page. Give a clean Arabic + English title for each chapter; invent a fitting one if the text has none.",
        items: {
          type: "object",
          properties: {
